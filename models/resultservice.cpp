...
void
ResultService::show(int electionId)
{
  ...
  std::transform(pairwiseContestList.begin(),
                 pairwiseContestList.end(),
  ...
  for (auto it = pairwiseResults.begin(); it != pairwiseResults.end(); it++) {
    auto optionAIter = std::find_if(
      optionList.begin(), optionList.end(), [&it](const Option& o) {
        return o.id() == (*it)["optionAId"];
      });
    auto optionBIter = std::find_if(
      optionList.begin(), optionList.end(), [&it](const Option& o) {
        return o.id() == (*it)["optionBId"];
      });
    if (optionAIter != optionList.end() && optionBIter != optionList.end()) {
      ...
      (*it)["winnerId"] =
        (*it)["optionACount"].toInt() > (*it)["optionBCount"].toInt()
          ...
          : (*it)["optionBId"];
      (*it)["delta"] =
        std::abs((*it)["optionACount"].toInt() - (*it)["optionBCount"].toInt());
    } else {
...
int
ResultService::create(int electionId)
{
