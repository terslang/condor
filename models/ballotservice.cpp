...
void
BallotService::show(const QString& id)
{
  ...
  std::transform(ballotchoice.begin(),
                 ballotchoice.end(),
  ...
  for (auto it = choices.begin(); it != choices.end(); it++) {
    auto optionIter =
      std::find_if(options.begin(), options.end(), [&it](const Option& o) {
        return o.id() == (*it)["optionId"];
      });
...
void
BallotService::edit(TSession& session, const QString& id)
{
...
int
BallotService::save(THttpRequest& request, TSession& session, const QString& id)
{
  auto model = Ballot::get(id);
...
bool
BallotService::remove(const QString& id)
{
  auto ballot = Ballot::get(id);
