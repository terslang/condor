...
void BallotChoiceService::show(const QString &ballotId)
{
    auto ballotChoice = BallotChoice::get(ballotId);
    texport(ballotChoice);
}

int BallotChoiceService::createMany(THttpRequest &request, TSession& session, const QString& ballotId)
{
    auto model = Ballot::get(ballotId);
    
    if (model.isNull()) {
        QString error = "Original ballot not found. It may have been updated/removed by another transaction.";
        tflash(error);
        return 0;
    }
...
QString BallotChoiceService::create(THttpRequest &request)
{
...
void BallotChoiceService::edit(TSession& session, const QString &ballotId)
{
...
void BallotChoiceService::showOptions(TSession &session, const QString &ballotId) {
    auto model = Ballot::get(ballotId);
...
int BallotChoiceService::save(THttpRequest &request, TSession &session, const QString &ballotId)
{
    auto model = BallotChoice::get(ballotId);
...
bool BallotChoiceService::remove(const QString &ballotId)
{
    auto ballotChoice = BallotChoice::get(ballotId);
