...
QString Option::body() const
{
    return d->body;
}

void Option::setBody(const QString &body)
{
    d->body = body;
}

int Option::electionId() const
{
    return d->election_id;
}

void Option::setElectionId(int electionId)
{
...
Option Option::create(const QString &name, const QString &body, int electionId)
{
    OptionObject obj;
    obj.name = name;
    obj.body = body;
...
Option Option::create(const QVariantMap &values)
{
    Option model;
...
Option Option::get(const QString &id)
{
...
QList<Option> Option::getAll()
{
    return tfGetModelListByCriteria<Option, OptionObject>(TCriteria());
}

QJsonArray Option::getAllJson(const QStringList &properties)
{
...
const TModelObject *Option::modelData() const
{
    return d.data();
}

QDataStream &operator<<(QDataStream &ds, const Option &model)
{
...
QDataStream &operator>>(QDataStream &ds, Option &model)
{
    QVariantMap varmap;
    ds >> varmap;
