...
User
User::authenticate(const QString& email, const QString& password)
{
...
User
User::create(const QString& email, const QString& password)
{
  UserObject obj;
  obj.email = email;
...
User
User::create(const QVariantMap& values)
{
  User model;
...
QJsonArray
User::getAllJson(const QStringList& properties)
{
...
const TModelObject*
User::modelData() const
{
  return d.data();
}

QDataStream&
operator<<(QDataStream& ds, const User& model)
{
...
QDataStream&
operator>>(QDataStream& ds, User& model)
{
  QVariantMap varmap;
  ds >> varmap;
