...
void
ResultController::show(const QString& id)
{
...
void
ResultController::create(const QString& id)
{
    ...
    case Tf::Post: {
      ...
      for (qsizetype i = 0; i < options.size(); i++) {
        for (qsizetype j = i + 1; j < options.size(); j++) {
          int res = pairwiseContestService.create(
            electionId, options[i].id(), options[j].id());
          if (res < 1) {
            rollbackTransaction();
